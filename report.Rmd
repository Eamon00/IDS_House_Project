...
main_colour <- "#3B82F6"   # Modern blue
accent_colour <- "#1E40AF" # Darker blue for accents
```
...
```{r, echo=FALSE}
ggplot(house_prices_filtered, aes(x = SalePrice)) +
  geom_histogram(
    bins = 30,
    fill = main_colour,
    color = accent_colour
...
Using the previously mentioned correlation heat-map, we condensed our number of variables to look at down to 17, by only selecting variables with a correlation of 0.15. We then decided to run the model on every variation of 12 variables, selecting the one that had the highest R^2 score. This allowed us to systematically reduce the number of variables down further, to only the variables with the highest prediciting power. We used a linear regression model, as its ouput is far more suitable for the data type than a logistic regression model, which is better for binary outputs.
...
# Plot SalePrice vs OverallCond with trend line
ggplot(df, aes(x = OverallCond, y = SalePrice)) +
  geom_jitter(alpha = 0.5, width = 0.2) +   # jitter to avoid overlapping points
