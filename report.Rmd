...
The dataset is published on Kaggle.^[Kaggle.com. (2016). Retrieved from https://www.kaggle.com/c/house-prices-advanced-regression-techniques]

The dataset is commonly used as a benchmark dataset for regression modelling ^[De Cock, D. (2011). Ames, Iowa: Alternative to the Boston Housing Data. *Journal of Statistics Education*, 19(3). Retrieved November 1, 2025, from https://jse.amstat.org/v19n3/.]
...
We applied ordinal numerical encoding to all of the quality-related variables. These variables describe quality using a set of ranked categories—such as Poor, Fair, Typical, Good, and Excellent which form an ordered scale. Since these categories represent increasing levels of quality, treating them as simple nominal labels would discard meaningful information about their ordering. By converting them to a 1–5 numerical scale, we preserve this hierarchy and allow the regression model to make use of the fact that, for exmaple, “Excellent” should indicate higher expected value than “Fair” or “Typical.” Ordinal encoding is therefore the most appropriate representation for quality-based variables of this type.
...
# scatter plot of Sale Price vs Total Bath with a fitted regression line.
```{r, echo=FALSE, message=FALSE, warning=FALSE}
library(tidyverse)
library(scales)

df <- read.csv("C:/Users/innes/OneDrive/Desktop/IDS_House_Project/data/train.csv")

df <- df %>% filter(!is.na(SalePrice))

if(!"TotalBath" %in% colnames(df)) {
  df <- df %>% mutate(TotalBath = FullBath + HalfBath * 0.5)
}

ggplot(df, aes(x = TotalBath, y = SalePrice)) +
  geom_point(color = "darkgreen", alpha = 0.6) +
  geom_smooth(method = "lm", color = "red", se = FALSE) +
  scale_y_continuous(labels = scales::comma) +
  labs(title = "Sale Price vs Total Bathrooms",
       x = "Total Bathrooms",
       y = "Sale Price") +
  theme_minimal()
```

# scatter plot of Sale Price vs Overall Quality with a fitted regression line.
```{r, echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
library(dplyr)
library(scales)
...
ggplot(df, aes(x = OverallQual, y = SalePrice)) +
  ...
  scale_y_continuous(labels = scales::comma) +
  labs(x = "Overall Quality (1–10)", y = "Sale Price ($)", title = "Sale Price vs Overall Quality") +
  theme_minimal()
...
## Figure 1.7, 1.8 and 1.9 - Plots produced by our model:
