---
title: "Untitled"
author: "eamon"
date: "2025-10-17"
output: html_document
---

```{r setup, include=FALSE}
library(readr)
library(tidyverse)
library(ggplot2)
house_prices <- read_csv("data/train.csv")

summary(house_prices)

```

# Cleaning done so far:
## • TotalBath = combination of all baths.
## • Kitchen Qual = changed to numerical values.
## • Extr Qual // 5
## • Pool area: if N/A -> 0
## • Paved drive -> changed to yes = 1


## R Markdown.

## Including Plots
```{r}
library(readr)
library(dplyr)

#CLEANING

house_prices <- read_csv("data/train.csv")

selected_var <- house_prices %>% 
  mutate(TotalBath = BsmtFullBath + 
           BsmtHalfBath + 
           HalfBath + 
           FullBath) %>% 
  select(Id,GarageCars,BedroomAbvGr,TotalBath, BldgType, LotArea, Neighborhood, KitchenQual,PavedDrive, TotalBsmtSF, Electrical, OverallQual,ExterQual, YearBuilt, GrLivArea, TotRmsAbvGrd, SalePrice)

# Numeric maps (Higher # = better)

qual_map <- c("Ex" = 5,
              "Gd" = 4,
              "TA" = 3,
              "Fa" = 2,
              "Po" = 1)

# Apply mapping
selected_var <- selected_var
selected_var$KitchenQual <- qual_map[selected_var$KitchenQual]
selected_var$ExterQual <- qual_map[selected_var$ExterQual]

qual_map <- c("Ex" = 5,
              "Gd" = 4,
              "TA" = 3,
              "Fa" = 2,
              "Po" = 1)

# Apply mapping
selected_var$KitchenQual <- qual_map[house_prices_sel$KitchenQual]
selected_var$ExterQual <- qual_map[house_prices_sel$ExterQual]
selected_var$HeatingQC <- qual_map[house_prices_sel$HeatingQC]
selected_var$HeatingQC <- qual_map[house_prices_sel$HeatingQC]

selected_var <- selected_var %>%
  mutate(PavedDrive = if_else(PavedDrive == "Y", 1, 0))
```


```{r}
# Load ggplot2
library(ggplot2)

# Fit linear regression
model_lm <- lm(SalePrice ~ ., data = selected_var)

# Predict
pred <- predict(model_lm, newdata = selected_var)

# Create a data frame for plotting
plot_data <- data.frame(
  Actual = selected_var$SalePrice,
  Predicted = pred
)

# Scatter plot of Actual vs Predicted
ggplot(plot_data, aes(x = Actual, y = Predicted)) +
  geom_point(color = "blue", alpha = 0.6) +   # points
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") + # perfect prediction line
  labs(title = "Predicted vs Actual SalePrice",
       x = "Actual SalePrice",
       y = "Predicted SalePrice") +
  theme_minimal()



```
```{r}


library(ggplot2)
library(dplyr)
library(purrr)
library(tidyr)


# Remove Id column from predictors
house_data <- selected_var %>% select(-Id)

# Identify predictor names
all_predictors <- setdiff(names(house_data), "SalePrice")

length(all_predictors)  # should show 15 predictors

# Generate all 12-column combinations
predictor_sets <- combn(all_predictors, 12, simplify = FALSE)

results <- list()
plot_list <- list()

for (i in seq_along(predictor_sets)) {
  
  preds <- predictor_sets[[i]]
  
  # Build formula
  form <- as.formula(
    paste("SalePrice ~", paste(preds, collapse = " + "))
  )
  
  model <- lm(form, data = house_data)
  pred <- predict(model, newdata = house_data)
  r2 <- summary(model)$r.squared
  
  # Save results
  results[[i]] <- list(
    predictors = preds,
    r2 = r2,
    model = model
  )
  
  # Residual plot
  resid_plot <- ggplot(
    data.frame(Residuals = model$residuals, Fitted = model$fitted.values),
    aes(x = Fitted, y = Residuals)
  ) +
    geom_point(alpha = 0.6) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
    labs(
      title = paste("Residual Plot for Model", i),
      subtitle = paste("Predictors:", paste(preds, collapse = ", ")),
      x = "Fitted Values",
      y = "Residuals"
    ) +
    theme_minimal()
  
  plot_list[[i]] <- resid_plot
  
  # Print progress
  message("Completed model ", i, " / ", length(predictor_sets), "   R2 = ", round(r2, 4))
}

# Find the best model
best_index <- which.max(map_dbl(results, "r2"))
best_model_info <- results[[best_index]]

cat("Best model is model", best_index, "\n")
cat("R² =", best_model_info$r2, "\n")
cat("Predictors used:\n")
print(best_model_info$predictors)

# Display the residual plot for the best model
plot_list[[best_index]]

# Use the best model to generate final predictions

final_model <- best_model_info$model

final_predictions <- predict(final_model, newdata = house_data)

output_df <- data.frame(
  Id = house_prices_filtered$Id,
  Actual_SalePrice = house_prices_sel$SalePrice,
  Predicted_SalePrice = final_predictions
)

# Save to CSV
write.csv(output_df, "final_predictions.csv", row.names = FALSE)

cat("Final predictions saved to final_predictions.csv\n")






```
```{r}
ggplot(house_prices, aes(x = OverallCond, y = SalePrice)) +
  geom_point(color = "steelblue", alpha = 0.6) +  # Scatter points
  geom_smooth(method = "lm", se = TRUE, color = "darkred") +  # Linear regression line
  labs(
    title = "Sale Price vs Overall Condition",
    x = "Overall Condition",
    y = "Sale Price"
  ) +
  theme_minimal()

```


